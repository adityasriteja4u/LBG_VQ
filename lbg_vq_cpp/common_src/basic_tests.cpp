...
const char* WRK_FILE_NAME = "length_8_book.lbg";
...
void basic_data_set_constructor_tst()
{
        ...
        try {
                ...
                LbgCodeBook<float, vec_len> res = LbgTrainer::TrainBook<float, vec_len>(dataset, 8, 0.1);
				trace_code_book<float, vec_len>(res);

				LbgBookToFile<float, vec_len>::WriteCodeBookToFile(WRK_FILE_NAME, res);
...
void basic_codebook_from_file_tst()
{
	printf(" ******* RUNNING basic_codebook_from_file_tst() ... ****** \n");

	const size_t vec_len = 13;

	LbgCodeBook<float, vec_len> back = LbgBookToFile<float, vec_len>::ReadBookFromFile(WRK_FILE_NAME);
	trace_code_book<float, vec_len>(back);

	printf(" ******* END OF basic_codebook_from_file_tst() ... ****** \n\n\n");
}
